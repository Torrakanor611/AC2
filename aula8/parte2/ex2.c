...
void _int_(12) isr_T3(void){
    ...
    if(x == 100){
        ...
        unsigned char dec = 0x30 | ((voltage & 0xF0) >> 4);
        putc(dec);

        unsigned char uni = 0x30 | (voltage & 0x0F);
        putc(uni);
...
void _int_(24) isr_uart1(void){
    ...
    if(IFS0bits.U1RXIF == 1){
        if(U1RXREG == 'L'){
            ...
            putString("minVolt: ");
            dec = (unsigned char) (int) voltMin / 10;
            uni = (char) (int) voltMin % 10;

            if(dec == ' '){
                putString("Estou aqui!");
            }
